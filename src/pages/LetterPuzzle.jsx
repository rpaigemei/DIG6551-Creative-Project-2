...
export function LetterPuzzle({ onSolved }) {
  const [clues, setClues] = useState([
    ...
    {id: 4, clue: "“What goes _____ must come down”", answer: "up", solved: false},
    ...
    {id: 9, clue: "Please --> _____ <-- and see", answer: "wait", solved: false},
  ]);
  ...
  useEffect(() => {
    const answer = clues.find(c => c.answer === guess && !c.solved);
  ...
  const checkSolved = (id) => {
    ...
    return <span className="answer">{clue.answer}</span>
  }
    ...
    <div className="puzzle-page">
      ...
      <div className="puzzle">
        <div className="letter">
          ...
          <div className="br">
            I can’t stop thinking about {checkSolved(1)}... I can hardly wait. I’ve finally found a place 
            that’s completely ours, a {checkSolved(2)} little spot where no one else will bother us, 
            Just for the two of us, finally. When you get to the {checkSolved(3)}, go {checkSolved(4)}stairs, 
            down the {checkSolved(5)} hallway, and through the {checkSolved(6)}. There’s a small room tucked  
            away that most people would never notice. I found it by accident and kept thinking how {checkSolved(7)} 
            it would be for us. I keep imagining us there, {checkSolved(8)}, talking, just having a world of our own.
          </div>
          <div>
        ...
        <input className="input" type="text" value={guess} onChange={handleChange} disabled={solved ? true : false} />
      </div>
    </div>
  )
}
